    ...
    stages {
        ...
        stage('Run ctest') {
            steps {
                sh '''
                    ...
                    docker run --rm -v $PWD/tmp_docker:/tmp --name daily-node-ctest-$DOCKER_BRANCH_TAG ${IMAGE}-${DOCKER_BRANCH_TAG}-${BUILD_NUMBER}-ctest sh -c \
        ...
        stage('Smoke Test') {
            steps {
                ...
                '''
                sh 'docker run --rm -d --name daily-node-smoke-test-${DOCKER_BRANCH_TAG} --net smoke-test-net-${DOCKER_BRANCH_TAG} ${IMAGE}-${DOCKER_BRANCH_TAG}-${BUILD_NUMBER} single'
                sh '''
                    ...
                    http_code=$(docker run --rm --net smoke-test-net-${DOCKER_BRANCH_TAG}  -v $PWD/test_build-d:/data byrnedo/alpine-curl \
                                       ...
                                       --url daily-node-smoke-test-${DOCKER_BRANCH_TAG}:7777 \
                                       -d '{
                                                "jsonrpc": "2.0",
                                                "id":"0",
            ...
            post {
                always {
                    sh 'docker kill daily-node-smoke-test-${DOCKER_BRANCH_TAG} || true'
