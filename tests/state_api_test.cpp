...
TEST_F(StateAPITest, DISABLED_eth_mainnet_smoke) {
  auto test_blocks =
      parse_rlp_file<vector<TestBlock>>(path(__FILE__).parent_path().parent_path() / "submodules" / "daily-evm" /
                                        "daily" / "data" / "eth_mainnet_blocks_0_300000.rlp");

  Config chain_config;
  auto initial_balances_rlp_hex_c = daily_evm_mainnet_initial_balances();
