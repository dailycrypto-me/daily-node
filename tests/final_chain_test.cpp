...
using namespace daily::core_tests;
...
struct FinalChainTest : WithDataDir {
  ...
  void create_validators() {
    ...
    for (const auto& keys : {dag_proposer_keys, pbft_proposer_keys}) {
      const auto vrf_pub_key = daily::vrf_wrapper::getVrfKeyPair().first;
  ...
  void fillConfigForGenesisTests(const addr_t& init_address) {
    ...
    cfg.genesis.state.initial_balances[init_address] = 1000000000 * kOneDly;
    cfg.genesis.state.dpos.eligibility_balance_threshold = 100000 * kOneDly;
    cfg.genesis.state.dpos.vote_eligibility_balance_step = 10000 * kOneDly;
    cfg.genesis.state.dpos.validator_maximum_stake = 10000000 * kOneDly;
    cfg.genesis.state.dpos.minimum_deposit = 1000 * kOneDly;
    cfg.genesis.state.dpos.eligibility_balance_threshold = 1000 * kOneDly;
...
TEST_F(FinalChainTest, initial_balances) {
  cfg.genesis.state.initial_balances = {};
  cfg.genesis.state.initial_balances[addr_t::random()] = daily::uint256_t("0x16345785D8A0000");    // 1
  cfg.genesis.state.initial_balances[addr_t::random()] = daily::uint256_t("0x56BC75E2D63100000");  // 1k
  cfg.genesis.state.initial_balances[addr_t::random()] =
      daily::uint256_t("0x204FCE5E3E25026110000000");  //  10 Billion
  init();
}
...
TEST_F(FinalChainTest, contract) {
  ...
  cfg.genesis.state.initial_balances = {};
  cfg.genesis.state.initial_balances[addr] = daily::uint256_t("0x204FCE5E3E25026110000000");  //  10 Billion
  init();
  auto nonce = 0;
...
TEST_F(FinalChainTest, coin_transfers) {
  ...
  for (size_t i = 0; i < NUM_ACCS; ++i) {
    ...
    cfg.genesis.state.initial_balances[k.address()] =
        daily::uint256_t("0x204FCE5E3E25026110000000") /* 10 Billion */ / NUM_ACCS;
  }
...
TEST_F(FinalChainTest, remove_jailed_validator_votes_from_total) {
  ...
  const auto [vrf_key, vrf_sk] = daily::vrf_wrapper::getVrfKeyPair();
