...
struct NodesTest : virtual WithDataDir {
  ...
  std::vector<daily::FullNodeConfig> make_node_cfgs(size_t total_count, size_t validators_count = 1,
                                                     uint tests_speed = 1, bool enable_rpc_http = false,
                                                     bool enable_rpc_ws = false);

  bool wait_connect(const std::vector<std::shared_ptr<daily::AppBase>>& nodes);
  ...
  std::vector<std::shared_ptr<AppBase>> launch_nodes(const std::vector<daily::FullNodeConfig>& cfgs);

  std::vector<daily::FullNodeConfig> node_cfgs;
};
