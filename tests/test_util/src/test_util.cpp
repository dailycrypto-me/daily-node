...
NodesTest::NodesTest() {
  for (uint16_t i = 0; i < 5; ++i) {
    daily::FullNodeConfig cfg;

    cfg.data_path = "/tmp/daily" + std::to_string(i);
    cfg.db_path = cfg.data_path / "db";
    cfg.log_path = cfg.data_path / "log";
    daily::logger::Config log_cfg(cfg.log_path);
    log_cfg.verbosity = daily::logger::Verbosity::Error;
    ...
    cfg.wallets.emplace_back(std::move(node_secret), daily::vdf_sortition::getVrfKeyPair().second);
    ...
    cfg.network.boot_nodes.emplace_back(
        daily::NodeConfig{"7b1fcf0ec1078320117b96e9e9ad9032c06d030cf4024a598347a4623a14a421d4f030cf25ef368ab394a45e9"
...
std::vector<daily::FullNodeConfig> NodesTest::make_node_cfgs(size_t total_count, size_t validators_count,
                                                              uint tests_speed, bool enable_rpc_http,
                                                              bool enable_rpc_ws) {
  std::vector<daily::FullNodeConfig> ret_configs = node_cfgs;
  ...
  daily::state_api::BalanceMap initial_balances;
  std::vector<daily::state_api::ValidatorInfo> initial_validators;
  ...
  const daily::uint256_t yield{20};  // [%]
  const daily::uint256_t hundred{100};
  const daily::uint256_t num_of_nodes{total_count};
  const daily::uint256_t max_supply = ret_configs.back().genesis.state.hardforks.aspen_hf.max_supply;

  const daily::uint256_t init_balance = (hundred * max_supply) / ((hundred + yield) * num_of_nodes);
  ...
  for (size_t idx = 0; idx < total_count; idx++) {
    ...
    daily::state_api::BalanceMap delegations;
    delegations.emplace(node_addr, cfg.genesis.state.dpos.eligibility_balance_threshold);
    initial_validators.emplace_back(daily::state_api::ValidatorInfo{
        node_addr, node_addr, daily::vrf_wrapper::getVrfPublicKey(cfg.getFirstWallet().vrf_secret), 100, "", "",
        delegations});
  }
...
std::vector<std::shared_ptr<AppBase>> NodesTest::launch_nodes(std::vector<daily::TestConfig>& cfgs) {
...
std::vector<std::shared_ptr<AppBase>> NodesTest::launch_nodes(const std::vector<daily::FullNodeConfig>& cfgs) {
  auto test_cfgs = make_test_cfgs(cfgs);
