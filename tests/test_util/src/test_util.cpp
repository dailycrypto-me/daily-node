...
NodesTest::NodesTest() {
  for (uint16_t i = 0; i < 5; ++i) {
    ...
    cfg.vrf_secret = daily::vdf_sortition::getVrfKeyPair().second;
...
std::vector<daily::FullNodeConfig> NodesTest::make_node_cfgs(size_t total_count, size_t validators_count,
  ...
  for (size_t idx = 0; idx < total_count; idx++) {
    ...
    initial_validators.emplace_back(daily::state_api::ValidatorInfo{
        node_addr, node_addr, daily::vrf_wrapper::getVrfPublicKey(cfg.vrf_secret), 100, "", "", delegations});
  }
...
bool NodesTest::wait_connect(const std::vector<std::shared_ptr<daily::FullNode>>& nodes) {
...
shared_nodes_t NodesTest::create_nodes(uint count, bool start) {
  ...
  for (uint j = 0; j < node_count; ++j) {
    ...
    nodes.emplace_back(std::make_shared<daily::FullNode>(cfgs[j]));
...
shared_nodes_t NodesTest::launch_nodes(const std::vector<daily::FullNodeConfig>& cfgs) {
