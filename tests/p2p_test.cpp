...
class TestDailyCapability final : public dev::p2p::CapabilityFace {
 public:
  TestDailyCapability(daily::network::tarcap::TarcapVersion version) : version_(version) {}

  std::string name() const override { return ""; }
  daily::network::tarcap::TarcapVersion version() const override { return version_; }
 ...
 private:
  daily::network::tarcap::TarcapVersion version_{1};
};

std::shared_ptr<dev::p2p::Host> makeTestNode(unsigned short listenPort,
                                             std::vector<daily::network::tarcap::TarcapVersion> tarcap_versions,
  ...
  auto makeTestTarcaps = [tarcap_versions](std::weak_ptr<dev::p2p::Host>) {
    ...
    for (const auto &version : tarcap_versions) {
      tarcaps.emplace_back(std::make_shared<TestDailyCapability>(version));
    }

    return tarcaps;
  };

  return Host::make("DailyNode", makeTestTarcaps, dev::KeyPair::create(),
                    dev::p2p::NetworkConfig("127.0.0.1", listenPort, false, true), DailyNetworkConfig{},
                    state_file_path);
}
...
TEST_F(P2PTest, p2p_discovery) {
  ...
  DailyNetworkConfig daily_net_conf;
  daily_net_conf.is_boot_node = true;
  auto dummy_capability_constructor = [](auto /*host*/) { return Host::CapabilityList{}; };
  util::ThreadPool tp;
  auto bootHost = Host::make("DailyNode", dummy_capability_constructor, key, net_conf, daily_net_conf);
  ...
  for (int i = 0; i < NUMBER_OF_NODES; i++) {
    auto node = nodes.emplace_back(Host::make("DailyNode", dummy_capability_constructor, dev::KeyPair::create(),
...
TEST_F(P2PTest, multiple_capabilities) {
  ...
  daily_net_conf.is_boot_node = true;
  auto boot_node =
      Host::make("DailyNode", [](auto /*host*/) { return Host::CapabilityList{}; }, key, net_conf, daily_net_conf);
  ...
  auto test_tarcaps = [boot_node_key](std::vector<daily::network::tarcap::TarcapVersion> node1_tarcap_versions,
                                      std::vector<daily::network::tarcap::TarcapVersion> node2_tarcap_versions,
