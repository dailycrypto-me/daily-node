...
TEST_F(PillarChainTest, block_serialization) {
  {
    ...
    // This hardcoded hex string is from bridge solidity test(DlyClient.t.sol/test_blockEncodeDecode)
  ...
  for (auto& node : nodes) {
    for (auto epoch = 0; epoch < 4; epoch++) {
      ...
      // check system transactions
      {
        ...
        ASSERT_EQ(trx->getSender(), kDailySystemAccount);
        ...
        ASSERT_EQ(trx_by_hash->getSender(), kDailySystemAccount);
