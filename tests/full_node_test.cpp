...
TEST_F(FullNodeTest, persist_counter) {
  ...
  unsigned long num_exe_trx1 = 0, num_exe_trx2 = 0, num_exe_blk1 = 0, num_exe_blk2 = 0, num_trx1 = 0, num_trx2 = 0;
  {
    ...
    for (unsigned i = 0; i < SYNC_TIMEOUT; i++) {
      ...
      daily::thisThreadSleepForMilliSeconds(200);
      num_exe_trx1 = nodes[0]->getDB()->getNumTransactionExecuted();
      num_exe_trx2 = nodes[1]->getDB()->getNumTransactionExecuted();
    }
...
TEST_F(FullNodeTest, save_network_to_file) {
  ...
  }
  {
    ...
    for (unsigned i = 0; i < SYNC_TIMEOUT; i++) {
      daily::thisThreadSleepForSeconds(1);
...
TEST_F(FullNodeTest, SoleiroliaHardfork) {
      ...
      "667358221220dc9174b3e76a9793588e608e7eb83322e3c86a0cac30ca520a374dc4e45cdc4d64736f6c634300081c0033";
  {
    ...
    EXPECT_EQ(node0->getTransactionManager()
                  ->estimateTransactionGas(trx2, node0->getFinalChain()->lastBlockNumber())
                  .gas_used,
              0);
  }
  CleanupDirs();
...
TEST_F(FullNodeTest, graphql_test) {
  ...
  auto q = std::make_shared<graphql::daily::Query>(nodes[0]->getFinalChain(), nodes[0]->getDagManager(),
  ...
  auto mutation = std::make_shared<graphql::daily::Mutation>(nodes[0]->getTransactionManager());
  auto subscription = std::make_shared<graphql::daily::Subscription>();
  auto _service = std::make_shared<graphql::daily::Operations>(q, mutation, subscription);
