...
TransactionManager::TransactionManager(FullNodeConfig const &conf, std::shared_ptr<DbStorage> db,
  ...
  LOG_OBJECTS_CREATE("TRXMGR");
  {
    ...
    trx_count_ = db_->getStatusField(daily::StatusDbField::TrxCount);
  }
}
...
TransactionStatus TransactionManager::insertValidatedTransaction(std::shared_ptr<Transaction> &&tx,
  ...
  const auto account = final_chain_->get_account(tx->getSender()).value_or(daily::state_api::ZeroAccount);
...
void TransactionManager::saveTransactionsFromDagBlock(SharedTransactions const &trxs) {
...

  {
    ...
    for (auto t : trxs) {
      const auto account = final_chain_->get_account(t->getSender()).value_or(daily::state_api::ZeroAccount);
