...
void to_str(daily_evm_Bytes b, std::string& result) { result = {reinterpret_cast<char*>(b.Data), b.Len}; }
 ...
 public:
  ...
  daily_evm_BytesCallback const cgo_part_{
      ...
      [](auto self, auto err_bytes) {
        ...
        daily_evm_traceback(decoder_cb_c<std::string, to_str>(traceback));
        msg += "\nGo stack trace:\n" + traceback;
...
bytes StateAPI::trace(EthBlockNumber blk_num, const EVMBlock& blk, const std::vector<EVMTransaction>& state_trxs,
  ...
  return c_method_args_rlp<bytes, from_rlp, daily_evm_state_api_trace_transactions>(this_c_, blk_num, blk, state_trxs,
                                                                                     trxs, params);
}
