...
FinalChain::FinalChain(const std::shared_ptr<DbStorage>& db, const daily::FullNodeConfig& config,
...
SharedTransaction FinalChain::makeBridgeFinalizationTransaction() {
  ...
  auto account = getAccount(kDailySystemAccount).value_or(state_api::ZeroAccount);
