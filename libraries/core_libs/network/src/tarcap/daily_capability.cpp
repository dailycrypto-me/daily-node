...
std::string DailyCapability::name() const { return DAILY_CAPABILITY_NAME; }

TarcapVersion DailyCapability::version() const { return version_; }

unsigned DailyCapability::messageCount() const { return SubprotocolPacketType::kPacketCount; }

void DailyCapability::onConnect(std::weak_ptr<dev::p2p::Session> session, u256 const &) {
...
void DailyCapability::onDisconnect(dev::p2p::NodeID const &_nodeID) {
...
std::string DailyCapability::packetTypeToString(unsigned _packetType) const {
  return convertPacketTypeToString(static_cast<SubprotocolPacketType>(_packetType));
}

void DailyCapability::interpretCapabilityPacket(std::weak_ptr<dev::p2p::Session> session, unsigned _id,
                                                 dev::RLP const &_r) {
  const auto session_p = session.lock();
  if (!session_p) {
    LOG(log_er_) << "Unable to obtain session ptr !";
...
void DailyCapability::handlePacketQueueOverLimit(std::shared_ptr<dev::p2p::Host> host, dev::p2p::NodeID node_id,
...
inline bool DailyCapability::filterSyncIrrelevantPackets(SubprotocolPacketType packet_type) const {
  switch (packet_type) {
...
const std::shared_ptr<PeersState> &DailyCapability::getPeersState() { return peers_state_; }

const DailyCapability::InitPacketsHandlers DailyCapability::kInitLatestVersionHandlers =
...
const DailyCapability::InitPacketsHandlers DailyCapability::kInitV4Handlers =
    [](const std::string &logs_prefix, const FullNodeConfig &config, const h256 &genesis_hash,
       const std::shared_ptr<PeersState> &peers_state, const std::shared_ptr<PbftSyncingState> &pbft_syncing_state,
       const std::shared_ptr<tarcap::TimePeriodPacketsStats> &packets_stats, const std::shared_ptr<DbStorage> &db,
       const std::shared_ptr<PbftManager> &pbft_mgr, const std::shared_ptr<PbftChain> &pbft_chain,
