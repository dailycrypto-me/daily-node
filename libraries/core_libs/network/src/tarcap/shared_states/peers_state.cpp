...
std::shared_ptr<DailyPeer> PeersState::getPeer(const dev::p2p::NodeID& node_id) const {
...
std::shared_ptr<DailyPeer> PeersState::getPendingPeer(const dev::p2p::NodeID& node_id) const {
  std::shared_lock lock(peers_mutex_);
...
std::pair<std::shared_ptr<DailyPeer>, std::string> PeersState::getPacketSenderPeer(
...
std::vector<dev::p2p::NodeID> PeersState::getAllPendingPeersIDs() const {
  ...
  std::transform(pending_peers_.begin(), pending_peers_.end(), std::back_inserter(peers),
                 [](std::pair<const dev::p2p::NodeID, std::shared_ptr<DailyPeer>> const& peer) { return peer.first; });
...
std::shared_ptr<DailyPeer> PeersState::addPendingPeer(const dev::p2p::NodeID& node_id, const std::string& address) {
  std::unique_lock lock(peers_mutex_);
  auto ret =
      pending_peers_.emplace(node_id, std::make_shared<DailyPeer>(node_id, kConf.transactions_pool_size, address));
  if (!ret.second) {
...
std::shared_ptr<DailyPeer> PeersState::setPeerAsReadyToSendMessages(dev::p2p::NodeID const& node_id,
...
std::shared_ptr<DailyPeer> PeersState::getMaxChainPeer(
    const std::shared_ptr<PbftManager> pbft_mgr, std::function<bool(const std::shared_ptr<DailyPeer>&)> filter_func) {
