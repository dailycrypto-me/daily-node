...
void StatusPacketHandler::process(StatusPacket&& packet, const std::shared_ptr<DailyPeer>& peer) {
  ...
  if (initial) {
    success = sealAndSend(
        ...
        encodePacketRlp(StatusPacket(
            ...
            StatusPacket::InitialData{kConf.genesis.chain_id, kGenesisHash, DAILY_MAJOR_VERSION, DAILY_MINOR_VERSION,
