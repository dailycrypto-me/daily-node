...
void StatusPacketHandler::process(const threadpool::PacketData& packet_data, const std::shared_ptr<DailyPeer>& peer) {
...
bool StatusPacketHandler::sendStatus(const dev::p2p::NodeID& node_id, bool initial) {
  ...
  LOG(log_dg_) << "Sending " << status_packet_type << " status message to " << node_id << ", protocol version "
               << DAILY_NET_VERSION << ", network id " << kConf.genesis.chain_id << ", genesis " << kGenesisHash
               << ", node version " << DAILY_VERSION;
  ...
  if (initial) {
    success = sealAndSend(
        ...
        std::move(dev::RLPStream(kInitialStatusPacketItemsCount)
                  ...
                  << pbft_syncing_state_->isPbftSyncing() << pbft_round << DAILY_MAJOR_VERSION << DAILY_MINOR_VERSION
                  << DAILY_PATCH_VERSION << dag_mgr_->isLightNode() << dag_mgr_->getLightNodeHistory()));
  } else {
    success = sealAndSend(
        node_id, StatusPacket,
