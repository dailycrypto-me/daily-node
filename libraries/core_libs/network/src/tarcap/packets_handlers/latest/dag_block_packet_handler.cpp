...
void DagBlockPacketHandler::sendBlock(dev::p2p::NodeID const &peer_id, daily::DagBlock block,
...
void DagBlockPacketHandler::sendBlockWithTransactions(dev::p2p::NodeID const &peer_id, daily::DagBlock block,
                                                      const SharedTransactions &trxs) {
  std::shared_ptr<DailyPeer> peer = peers_state_->getPeer(peer_id);
  if (!peer) {
    LOG(log_wr_) << "Send dag block " << block.getHash() << ". Failed to obtain peer " << peer_id;
...
void DagBlockPacketHandler::onNewBlockReceived(
    DagBlock &&block, const std::shared_ptr<DailyPeer> &peer,
