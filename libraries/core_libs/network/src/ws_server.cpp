...
void WsServer::newEthBlock(const ::daily::final_chain::BlockHeader &header, const TransactionHashes &trx_hashes) {
...
void WsServer::newLogs(const ::daily::final_chain::BlockHeader &header, TransactionHashes trx_hashes,
                       const TransactionReceipts &receipts) {
  boost::shared_lock<boost::shared_mutex> lock(sessions_mtx_);
  if (sessions_.empty()) return;
