...
Network::Network(const FullNodeConfig &config, const h256 &genesis_hash, const std::filesystem::path &network_file_path,
  ...
  const std::string net_version = "DailyNode";
  ...
  dev::p2p::Host::CapabilitiesFactory constructCapabilities = [&](std::weak_ptr<dev::p2p::Host> host) {
    ...
    auto latest_tarcap = std::make_shared<network::tarcap::DailyCapability>(
        DAILY_NET_VERSION, config, genesis_hash, host, packets_tp_, all_packets_stats_, pbft_syncing_state_, db,
    ...
    // Register previous (v5) version of daily capability
    assert(DAILY_NET_VERSION - 1 == 5);
    auto v5_tarcap = std::make_shared<network::tarcap::DailyCapability>(
        DAILY_NET_VERSION - 1, config, genesis_hash, host, packets_tp_, all_packets_stats_, pbft_syncing_state_, db,
        pbft_mgr, pbft_chain, vote_mgr, dag_mgr, trx_mgr, slashing_manager, pillar_chain_mgr, final_chain,
        network::tarcap::DailyCapability::kInitV5VersionHandlers);
  ...
  host_ = dev::p2p::Host::make(net_version, constructCapabilities, dev::KeyPair(kConf.getFirstWallet().node_secret),
                               net_conf, daily_net_conf, network_file_path);
