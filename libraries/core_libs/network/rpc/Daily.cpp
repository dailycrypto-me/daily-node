...
Daily::Daily(std::shared_ptr<AppBase> app) : app_(app) {
...
std::shared_ptr<AppBase> Daily::tryGetApp() {
...
Json::Value Daily::daily_getNodeVersions() {
  try {
    auto app = tryGetApp();
    auto db = app->getDB();
...
Json::Value Daily::daily_getConfig() { return enc_json(tryGetApp()->getConfig().genesis); }
