...
class PacketsBlockingMask {
 ...
 private:
  ...
  std::optional<daily::level_t> getSmallestDagLevelBeingProcessed() const;
 ...
 private:
  ...
  std::map<daily::level_t, std::unordered_set<PacketData::PacketId>> processing_dag_levels_;
  ...
  std::map<daily::sig_t, PacketData::PacketId> processing_dag_blocks_;
