...
class Network {
 public:
  ...
  std::shared_ptr<network::tarcap::DailyPeer> getMaxChainPeer() const;
  ...
  std::shared_ptr<network::tarcap::DailyPeer> getPeer(dev::p2p::NodeID const &id) const;
 ...
 private:
  ...
  // All supported daily capabilities - in descending order
  std::map<network::tarcap::TarcapVersion, std::shared_ptr<network::tarcap::DailyCapability>,
