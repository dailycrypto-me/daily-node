...
template <typename PacketHandlerType>
std::shared_ptr<PacketHandlerType> DailyCapability::getSpecificHandler(SubprotocolPacketType packet_type) const {
  // Note: Allow to manually cast only to known base classes types.
  // We support multiple daily capabilities, which can contain different versions of packet handlers and casting
