...
class ExtSyncingPacketHandler : public PacketHandler<PacketType> {
 public:
  ...
  void requestPendingDagBlocks(std::shared_ptr<DailyPeer> peer = nullptr) {
    if (!peer) {
  ...
  std::shared_ptr<DailyPeer> getMaxChainPeer(std::function<bool(const std::shared_ptr<DailyPeer> &)> filter_func =
