...
class Block {
 public:
  explicit Block(std::shared_ptr<::daily::final_chain::FinalChain> final_chain,
                 ...
                 const ::daily::blk_hash_t& pbft_block_hash,
                 std::shared_ptr<const ::daily::final_chain::BlockHeader> block_header) noexcept;
 ...
 private:
  ...
  const ::daily::blk_hash_t kPBftBlockHash;
  std::shared_ptr<const ::daily::final_chain::BlockHeader> block_header_;
  mutable std::vector<std::shared_ptr<::daily::Transaction>> transactions_;
};
