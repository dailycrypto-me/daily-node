...
Block::Block(std::shared_ptr<::daily::final_chain::FinalChain> final_chain,
...
std::shared_ptr<object::Account> Block::getAccount(response::Value&& addressArg) const {
  return std::make_shared<object::Account>(
      std::make_shared<Account>(final_chain_, ::daily::addr_t(addressArg.get<std::string>()), block_header_->number));
}
