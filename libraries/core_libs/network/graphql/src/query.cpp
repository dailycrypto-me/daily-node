...
Query::Query(std::shared_ptr<::daily::final_chain::FinalChain> final_chain,
             std::shared_ptr<::daily::DagManager> dag_manager, std::shared_ptr<::daily::PbftManager> pbft_manager,
             std::shared_ptr<::daily::TransactionManager> transaction_manager, std::shared_ptr<::daily::DbStorage> db,
             std::shared_ptr<::daily::GasPricer> gas_pricer, std::weak_ptr<::daily::Network> network,
    ...
    : final_chain_(std::move(final_chain)),
      ...
      kChainId(chain_id) {
  get_block_by_num_ = [&](::daily::EthBlockNumber num) {
...
std::shared_ptr<object::Block> Query::getBlock(std::optional<response::Value>&& number,
  ...
  std::optional<::daily::EthBlockNumber> block_number;
  if (number) {
  ...
  if (block_number == 0) [[unlikely]] {
    return std::make_shared<object::Block>(std::make_shared<Block>(
        final_chain_, transaction_manager_, get_block_by_num_, ::daily::blk_hash_t(), block_header));
  }
...
std::shared_ptr<object::Transaction> Query::getTransaction(response::Value&& hashArg) const {
  if (auto transaction = transaction_manager_->getTransaction(::daily::trx_hash_t(hashArg.get<std::string>()))) {
...
std::shared_ptr<object::Account> Query::getAccount(response::Value&& addressArg,
  ...
  const auto address = ::daily::addr_t(addressArg.get<std::string>());
  if (blockArg) {
...
std::shared_ptr<object::DagBlock> Query::getDagBlock(std::optional<response::Value>&& hashArg) const {
  std::shared_ptr<::daily::DagBlock> daily_dag_block = nullptr;

  if (hashArg) {
    if (const auto hash = ::daily::blk_hash_t(hashArg->get<response::StringType>());
        hash != ::daily::kNullBlockHash) {
      daily_dag_block = dag_manager_->getDagBlock(hash);
    }
  } else {
    auto dag_blocks = db_->getDagBlocksAtLevel(dag_manager_->getMaxLevel(), 1);

    if (dag_blocks.size() > 0) {
      daily_dag_block = dag_blocks.front();
    }
  }
  if (daily_dag_block) {
    return std::make_shared<object::DagBlock>(std::make_shared<DagBlock>(
        std::move(daily_dag_block), final_chain_, pbft_manager_, transaction_manager_, get_block_by_num_));
  }
  return nullptr;
}
...
std::vector<std::shared_ptr<object::DagBlock>> Query::getDagBlocks(std::optional<response::Value>&& dagLevelArg,
  ...
  ::daily::level_t act_dag_level = dag_manager_->getMaxLevel();
  ...
  auto addDagBlocks = [final_chain = final_chain_, pbft_manager = pbft_manager_,
                       transaction_manager = transaction_manager_, get_block_by_num = get_block_by_num_](
                          auto daily_dag_blocks, auto& result_dag_blocks) -> size_t {
    for (auto& dag_block : daily_dag_blocks) {
