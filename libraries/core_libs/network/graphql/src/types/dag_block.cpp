...
DagBlock::DagBlock(std::shared_ptr<::daily::DagBlock> dag_block,
...
std::optional<response::Value> DagBlock::getPbftPeriod() const noexcept {
  ...
  const auto [has_period, period] = pbft_manager_->getDagBlockPeriod(::daily::blk_hash_t(dag_block_->getHash()));
  if (has_period) {
    period_ = period;
    return {response::Value(static_cast<int>(*period_))};
  }
