...
std::string GraphQlWsSession::processRequest(const std::string_view& /*request*/) {
  //  static auto q = std::make_shared<graphql::daily::Query>(ws_server_.lock()->getFinalChain(), nullptr, 0);
  //  static auto mutation = std::make_shared<graphql::daily::Mutation>();
  //  static auto subscription = std::make_shared<graphql::daily::Subscription>();
  //  static auto _service = std::make_shared<graphql::daily::Operations>(q, mutation, subscription);
  //
