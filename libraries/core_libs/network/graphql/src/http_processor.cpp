...
GraphQlHttpProcessor::GraphQlHttpProcessor(std::shared_ptr<::daily::final_chain::FinalChain> final_chain,
                                           ...
                                           std::shared_ptr<::daily::DbStorage> db,
                                           std::shared_ptr<::daily::GasPricer> gas_pricer,
                                           std::weak_ptr<::daily::Network> network, uint64_t chain_id)
    : HttpProcessor(),
      query_(std::make_shared<graphql::daily::Query>(std::move(final_chain), std::move(dag_manager),
                                                      std::move(pbft_manager), transaction_manager, std::move(db),
                                                      std::move(gas_pricer), std::move(network), chain_id)),
      mutation_(std::make_shared<graphql::daily::Mutation>(transaction_manager)),
      subscription_(std::make_shared<graphql::daily::Subscription>()),
