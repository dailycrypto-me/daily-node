...
DbStorage::DbStorage(fs::path const& path, uint32_t db_snapshot_each_n_pbft_block, uint32_t max_open_files,
  ...
  if (kMajorVersion_ != 0 && kMajorVersion_ != DAILY_DB_MAJOR_VERSION) {
    major_version_changed_ = true;
  } else if (minor_version != DAILY_DB_MINOR_VERSION) {
...
void DbStorage::updateDbVersions() {
  saveStatusField(StatusDbField::DbMajorVersion, DAILY_DB_MAJOR_VERSION);
  saveStatusField(StatusDbField::DbMinorVersion, DAILY_DB_MINOR_VERSION);
  kMajorVersion_ = DAILY_DB_MAJOR_VERSION;
}
...
void DbStorage::addPbftHeadToBatch(daily::blk_hash_t const& head_hash, std::string const& head_str,
                                   Batch& write_batch) {
...
void DbStorage::addPbftBlockPeriodToBatch(PbftPeriod period, daily::blk_hash_t const& pbft_block_hash,
                                          Batch& write_batch) {
  insert(write_batch, Columns::pbft_block_period, toSlice(pbft_block_hash.asBytes()), toSlice(period));
}

std::pair<bool, PbftPeriod> DbStorage::getPeriodFromPbftHash(daily::blk_hash_t const& pbft_block_hash) {
