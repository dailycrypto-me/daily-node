...
Host::Host(std::string _clientVersion, KeyPair const& kp, NetworkConfig _n, DailyNetworkConfig daily_conf,
           std::filesystem::path state_file_path)
    : ioc_(daily_conf.expected_parallelism),
      ioc_w_(ba::make_work_guard(ioc_)),
      session_ioc_(daily_conf.expected_parallelism),
      ...
      daily_conf_(std::move(daily_conf)),
      m_idealPeerCount(daily_conf_.ideal_peer_count),
      m_stretchPeers(daily_conf_.peer_stretch),
  ...
  assert(1 <= daily_conf.expected_parallelism);
  ...
  m_nodeTable = make_unique<NodeTable>(
      ...
      daily_conf_.is_boot_node, daily_conf_.chain_id);
...
std::shared_ptr<Host> Host::make(std::string _clientVersion, CapabilitiesFactory const& cap_factory, KeyPair const& kp,
                                 NetworkConfig _n, DailyNetworkConfig daily_conf,
                                 std::filesystem::path state_file_path) {
  shared_ptr<Host> self(
      new Host(std::move(_clientVersion), kp, std::move(_n), daily_conf, std::move(state_file_path)));
...
void Host::main_loop_body() {
  ...
  m_runTimer.expires_after(daily_conf_.main_loop_interval);
  m_runTimer.async_wait(ba::bind_executor(strand_, [this](...) { main_loop_body(); }));
}

void Host::keepAlivePeers() {
  if (chrono::steady_clock::now() - daily_conf_.peer_healthcheck_interval < m_lastPing) {
    return;
  }
...
void Host::logActivePeers() {
  if (chrono::steady_clock::now() - daily_conf_.log_active_peers_interval < m_lastPeerLogMessage) {
    return;
  }
