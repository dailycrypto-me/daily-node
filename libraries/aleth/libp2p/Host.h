...
struct Host final : std::enable_shared_from_this<Host> {
 ...
 private:
  Host(std::string _clientVersion, KeyPair const& kp, NetworkConfig _n, DailyNetworkConfig daily_conf,
 ...
 public:
  static std::shared_ptr<Host> make(std::string _clientVersion, CapabilitiesFactory const& cap_factory,
                                    KeyPair const& kp, NetworkConfig _n, DailyNetworkConfig daily_conf = {},
                                    std::filesystem::path state_file_path = {});
 ...
 private:
  ...
  DailyNetworkConfig daily_conf_;
