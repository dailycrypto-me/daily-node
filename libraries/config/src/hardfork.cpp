...
void dec_json(const Json::Value& json, Redelegation& obj) {
  obj.validator = daily::addr_t(json["validator"].asString());
  obj.delegator = daily::addr_t(json["delegator"].asString());
...
bool FicusHardforkConfig::isFicusHardfork(daily::PbftPeriod period) const { return period >= block_num; }

bool FicusHardforkConfig::isPillarBlockPeriod(daily::PbftPeriod period, bool skip_first_pillar_block) const {
...
bool FicusHardforkConfig::isPbftWithPillarBlockPeriod(daily::PbftPeriod period) const {
...
daily::PbftPeriod FicusHardforkConfig::firstPillarBlockPeriod() const {
...
void FicusHardforkConfig::validate(uint32_t delegation_delay) const {
  ...
  if (block_num < delegation_delay) {
    throw daily::ConfigException("ficus_hf.block_num must be >= dpos.delegation_delay");
  }

  if (pillar_blocks_interval <= 1) {
    throw daily::ConfigException("ficus_hf.pillar_blocks_interval must be > 1");
  }

  if (block_num % pillar_blocks_interval) {
    throw daily::ConfigException("ficus_hf.block_num % ficus_hf.pillar_blocks_interval must == 0");
  }
}
...
void dec_json(const Json::Value& json, FicusHardforkConfig& obj) {
  ...
  obj.bridge_contract_address = daily::addr_t(json["bridge_contract_address"].asString());
}
