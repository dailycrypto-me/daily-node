...
Json::Value overrideConfig(Json::Value& conf, std::string& data_dir, const vector<string>& boot_nodes,
  ...
  if (data_dir.empty()) {
    if (conf["data_path"].asString().empty()) {
      conf["data_path"] = getDailyDataDefaultDir();
    }
...
void generateWallet(const string& wallet) {
  ...
  auto [pk, sk] = daily::vrf_wrapper::getVrfKeyPair();
...
Json::Value overrideWallet(Json::Value& wallet, const std::string& node_key, const std::string& vrf_key) {
  ...
  if (!vrf_key.empty()) {
    auto sk = daily::vrf_wrapper::vrf_sk_t(vrf_key);
    auto pk = daily::vrf_wrapper::getVrfPublicKey(sk);
...
void generateVrfFromKey(const string& key) {
  auto sk = daily::vrf_wrapper::vrf_sk_t(key);
  auto pk = daily::vrf_wrapper::getVrfPublicKey(sk);
  generateVrf(sk, pk);
}
...
string getDailyDefaultDir() { return getHomeDir() + "/" + DEFAULT_DAILY_DIR_NAME; }

string getDailyDataDefaultDir() { return getHomeDir() + "/" + DEFAULT_DAILY_DATA_DIR_NAME; }

string getDailyDefaultWalletFile() { return getDailyDefaultDir() + "/" + DEFAULT_WALLET_FILE_NAME; }

string getDailyDefaultConfigFile() { return getDailyDefaultDir() + "/" + DEFAULT_CONFIG_FILE_NAME; }

string getDailyDefaultGenesisFile() { return getDailyDefaultDir() + "/" + DEFAULT_GENESIS_FILE_NAME; }
