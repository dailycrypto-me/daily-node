...
Config::Config(int argc, const char* argv[]) {
  ...
  config = tools::getDailyDefaultConfigFile();
  wallet = tools::getDailyDefaultWalletFile();
  genesis = tools::getDailyDefaultGenesisFile();

  // Define all the command line options and descriptions
  main_options.add_options()(HELP, "Print this help message and exit");
  main_options.add_options()(VERSION, bpo::bool_switch(&version), "Print version of dailyd");
  ...
  if (option_vars.count(HELP)) {
    std::cout << "NAME:\n  "
                 "dailyd - Daily blockchain full node implementation\n"
                 "VERSION:\n  "
              << DAILY_VERSION << "\nUSAGE:\n  dailyd [options]\n";
