...
//			b) replace " by \" and copy&paste the ABI string into the DailyDposClientMetaData
//		 2. To get implementation bytecode(DailyDposImplBytecode)
//			a) run `solc --bin-runtime --overwrite --optimize dpos_contract_impl.sol --output-dir .`
//			b) Copy bytecode from `DposDummyImpl.bin-runtime` file to `var DailyDposImplBytecode` variable in `dpos_contract_solidity_structs.go` file.
//		 2. To generate solidity interface related structs:
//		 	a) run `abigen --abi=DposInterface.abi --pkg=dailyDposClient --out=dpos_contract_interface.go`
