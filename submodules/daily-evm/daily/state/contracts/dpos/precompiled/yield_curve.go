...
// Yield curve description -> https://github.com/dailycrypto-me/TIP/blob/main/TIP-2/TIP-2%20-%20Cap%20DLY's%20Total%20Supply.md
...
func (self *YieldCurve) calculateCurrentYield(current_total_dly_supply *uint256.Int) *uint256.Int {
	// Current yield = (max supply - current total supply) / current total supply
	current_yield := new(uint256.Int).Sub(self.max_supply, current_total_dly_supply)
...
func (self *YieldCurve) CalculateBlockReward(current_total_delegation, current_total_dly_supply *uint256.Int) (block_reward *uint256.Int, yield *uint256.Int) {
	yield = self.calculateCurrentYield(current_total_dly_supply)
