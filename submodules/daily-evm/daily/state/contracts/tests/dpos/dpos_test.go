...
import (
	...
	"github.com/dailycrypto-me/daily-evm/accounts/abi"
	...
	test_utils "github.com/dailycrypto-me/daily-evm/daily/state/contracts/tests"
...
var (
	...
	DefaultBalance                     = bigutil.Mul(big.NewInt(2050000000), DlyPrecision)
	DefaultEligibilityBalanceThreshold = bigutil.Mul(big.NewInt(1000000), DlyPrecision)
	DefaultVoteEligibilityBalanceStep  = bigutil.Mul(big.NewInt(1000), DlyPrecision)
	DefaultValidatorMaximumStake       = bigutil.Mul(big.NewInt(10000000), DlyPrecision)
	DefaultMinimumDeposit              = bigutil.Mul(big.NewInt(1000), DlyPrecision)
	...
	DefaultChainCfg = chain_config.ChainConfig{
		...
		Hardforks: chain_config.HardforksConfig{
			...
			AspenHf: chain_config.AspenHfConfig{
				BlockNumPartOne: 0,
				BlockNumPartTwo: 0,
				// Max token supply is 12 Billion DLY -> 12e+9(12 billion) * 1e+18(dly precision)
...
func TestRegisterValidator(t *testing.T) {
	_, test := test_utils.Init_test(dpos.ContractAddress(), dpos_sol.DailyDposClientMetaData, t, CopyDefaultChainConfig())
	defer test.End()
...
func TestDelegateMinMax(t *testing.T) {
	...
	delegation := bigutil.Mul(big.NewInt(5000000), DlyPrecision)
...
func TestRedelegate(t *testing.T) {
	tc, test := test_utils.Init_test(dpos.ContractAddress(), dpos_sol.DailyDposClientMetaData, t, CopyDefaultChainConfig())
	defer test.End()
...
func TestPreMagnoliaHfUndelegate(t *testing.T) {
	...
	tc, test := test_utils.Init_test(dpos.ContractAddress(), dpos_sol.DailyDposClientMetaData, t, cfg)
	defer test.End()
	val_owner := addr(1)
	val_addr, proof := generateAddrAndProof()
...
func TestYieldCurveAspenHf(t *testing.T) {
	...
	// max supply has hardcoded value of 12 Billion DLY
...
func TestAspenHf(t *testing.T) {
	...
	cfg.Hardforks.AspenHf.GeneratedRewards = bigutil.Mul(big.NewInt(5000000), big.NewInt(1e18)) // 5M DLY
	...
	trxFee := bigutil.Div(DlyPrecision, big.NewInt(1000)) //  0.001 DLY
...
func TestSetCommission(t *testing.T) {
	...
	_, test := test_utils.Init_test(dpos.ContractAddress(), dpos_sol.DailyDposClientMetaData, t, cfg)
	defer test.End()
...
func TestGetValidators(t *testing.T) {
	...
	cfg := DefaultChainCfg
	validator_balance := bigutil.Mul(big.NewInt(100000000), DlyPrecision)
...
func TestMakeLogsCheckTopics(t *testing.T) {
	...
	Abi, _ := abi.JSON(strings.NewReader(dpos_sol.DailyDposClientMetaData))
